...
def dual_cost(k, m, p, z, t, delta, u, sei, verb):
	...
	#Success probability
	d = float(L_sht * sei) 
	if d < 50: #Depth 2 gives more testing candidates (typically 7**15, hence require lots of samples)
...
def dual_cost_v2(k, m, p, z, t, delta1, delta2, u, sei, verb):
	...
	L_t = (z)**t * comb(int(m),t)
	#2t-error sample list. 2 list here. Devided by z due to dependencies
	...
	lists =[log2(4*m), log2(L_t), log2(L_2t), log2(L)]
	...
	for delta1 in range (delta1_below,20):
		for delta2 in range(delta2_below,delta1+1):
			...
			for t in range(4,12):
				...
				L_shtmax = L_max * comb(u, int(u/2)) * 2**(-u) #To simplify, ignore the probability of weight u
				d_max = float(L_shtmax * sei)
				if d_max < 80:
...
if __name__=='__main__':
	...
	for exp_above in range(13,max_exp-2,1): #range (exp_below+3, max_exp-2)
		...
		if comp < NIST1:
			...
			if round(comp,2) not in C:
				...
				with open("outputd2.txt", "a") as file:
    				# Writing the C list
	...
	#print(dual_cost(k = 76, m = 2**(22.55), p = 127, z = 7, t = 6, delta = 22, u = 75, sei = SEI, verb = True))
	print('-----------------')
	#print(dual_cost_new(k = 76, m = 2**(16.56), p = 127, z = 7, t = 7, delta = 19, u = 75, sei = SEI, verb = True))
	
	print(dual_cost_v2(k=76, m = 2**(18.81), p = 127, z = 7, t = 6, delta1 = 17, delta2 = 17, u = 55, sei = SEI, verb = True))
